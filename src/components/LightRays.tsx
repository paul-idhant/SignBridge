...
/**
 * LightRays — volumetric god-rays rendered in a WebGL shader (ogl).
 * Ported to TypeScript for SignBridge. Optional cursor-follow makes the
 * rays lean toward the mouse like a spotlight. Renders only while the
 * container is on-screen (IntersectionObserver) and cleans up its GL
 * context on unmount.
 */
...
export interface LightRaysProps {
  ...
  distortion?: number;
  lightMode?: boolean;
  /** Overall brightness multiplier for the rays (1 = default). */
  intensity?: number;
  className?: string;
}
...
const LightRays = ({
  ...
  /* Only run the GL loop while on-screen (battery friendly) */
  useEffect(() => {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  fragColor.rgb *= raysColor * uIntensity;
...
void main() {
  ...
  /* Live uniform updates without re-initializing WebGL */
  useEffect(() => {
